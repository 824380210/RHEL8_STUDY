...
```
1： 认识linux目录结构是根状发散的，所有目录的最终上级是根目录,根目录用/表示
	1： /bin 是符号连接到 /usr/bin目录，一般是用户命令
	2： /sbin是符号连接到 /usr/sbin目录，一般是系统管理命令
	3： /lib 是符号连接到 /usr/lib目录，是库文件
	4:  /lib64 是符号连接到 /usr/lib64目录，是库文件

2: /etc目录是系统的配置文件

3： /var 保存用户的可变数据，如数据库，缓存，日志等

4： /tmp临时文件，10天末访问就会删除， /var/tmp是30天没有访问就删除

5： /boot是系统启动文件

7： 理解绝对路径与相对路径，cd命令的用法，cd .. / cd - 是回到命令在当前目录之前的一个目录，PWD来显示当前绝对路径

8； mkdir / mkdir -p 用法

9： cp -r / cp 用法  （-r是递归，通过一个命令复制多个文件的时候，最后一个参数一定是目录呵！）

10： mv 

11: rm / rm -f / rm -fr 用法，-f是force，不会提示直接删除，有风险呵，rmdir 仅删除空目录
